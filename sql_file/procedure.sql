...
DELIMITER $$
CREATE OR REPLACE DEFINER=`root`@`localhost` PROCEDURE `LOGIN`(IN `EMAIL` VARCHAR(100), IN `PASSWORD1` VARCHAR(100), OUT `STATUS` VARCHAR(100))
BEGIN
DECLARE UID VARCHAR(100); 
SET UID = NULL ; 
SELECT ID UID FROM USERS WHERE `EMAIL` = EMAIL AND `PASSWORD` = PASSWORD1 LIMIT 1 ;   
